...
A lista legkisebb eleme lambda: {min(lista, key=lambda x : x if x % 2 == 0 else 0)}
               sima  max {max(lista)}
a lista legkisebb eleme: {legkisebb}
A lista legkisebb eleme: {min(lista)}''')
...
A szavakat tárolja el a program egy listában! Az adatbekérés befejezte után írja ki a program
a lista elemeit, a legrövidebb és a leghosszabb szót!
"""
print("_____________________________________________________mimozaaaaaaa_________________________________________________________________")

lista: list = []


while True:
    bekeres = input("Kérek egy szavat:\t")
    if bekeres == "":
        break
    else:
...
A leghosszab szó: {legnagyobb}
A legrövidebb szó: {legkisebb}''')
