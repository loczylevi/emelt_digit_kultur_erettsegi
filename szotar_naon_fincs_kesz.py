...
betu_bekero = input("Adjon meg egy betűt!\t")


marka_tipus_kereso = [[sor["márka"], sor["tipus"]] for sor in kocsik if betu_bekero.lower() in sor["márka"].lower() or betu_bekero.lower() in sor["tipus"].lower()]


if len(marka_tipus_kereso) > 0:
    print(f'''A megadott betű az alábbi autók márka- vagy típusmegnevezésében fordul elő:''')
    for elem in marka_tipus_kereso:
        print(elem[0],elem[1])
else:
        print(f'''A megadott betű az alábbi autók márka- vagy típusmegnevezésében nem fordul elő!''')
